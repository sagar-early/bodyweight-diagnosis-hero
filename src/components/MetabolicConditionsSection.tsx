  ...
  return (
    <div className="w-full mb-8">
      ...
      <div className="text-center mb-8">
        <h1 className="font-unna text-2xl lg:text-4xl text-foreground mb-4">
          What's Really Behind Your Weight Gain?
        </h1>
        <h3 className="font-satoshi text-lg lg:text-xl text-secondary">
        ...
        {metabolicConditions.map((condition) => {
          ...
          return (
            <div key={condition.id} className="flex flex-col items-center text-center">
              <div className="w-12 h-12 mb-3 flex items-center justify-center rounded-full bg-accent/20">
                <IconComponent className="w-6 h-6 text-accent" />
              </div>
              <p className="font-satoshi text-sm text-foreground leading-tight">
                {condition.name}
              </p>
            </div>
          );
